...
// PE Parser "Windows portable executable"
// @maboalenen
...
int main(int argc, char* argv[]) {
	...
	for (int j = 1; j < argc ; j++) {
		memcpy_s(&fileName, MAX_FILEPATH, argv[j], MAX_FILEPATH);
		printf ("\n\n*********** %s ***********\n", fileName);
		HANDLE file = NULL;
		DWORD fileSize;
		DWORD bytesRead;
		...
		file = CreateFile(fileName,               // file to open
	                       ...
	                       OPEN_EXISTING,         // existin&byteNAMEsRead,
		...
		fileData = MapViewOfFile(fileMapping, FILE_MAP_READ, 0, 0, 0);
		if (fileData == 0) {
		...
		// IMAGE_NT_HEADERS
		imageNTHeaders = PIMAGE_NT_HEADERS((BYTE*)fileData + dosHeader->e_lfanew);
		...
		// FILE_HEADER
		fileHeader = (PIMAGE_FILE_HEADER)&imageNTHeaders->FileHeader;
		printf("\n******* FILE HEADER *******\n");
		printf("\t0x%x\t\tMachine\n", fileHeader->Machine);
		printf("\t0x%d\t\tNumber of Sections\n", fileHeader->NumberOfSections);
		printf("\t0x%x (%d)\t\tTime Stamp\n", fileHeader->TimeDateStamp, fileHeader->TimeDateStamp);
		...
		BYTE* sectionLocation = (BYTE*)imageNTHeaders + sizeof(DWORD) + (DWORD)(sizeof(IMAGE_FILE_HEADER)) + (DWORD)imageNTHeaders->FileHeader.SizeOfOptionalHeader;
		...
		rawOffset = (BYTE*)fileData + importSection->PointerToRawData;
